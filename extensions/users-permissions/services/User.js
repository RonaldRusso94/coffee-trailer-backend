...
module.exports = {
    async fetchAuthenticatedUser(id) {
        debugger
        const user =  await strapi.query("user", "users-permissions").findOne({ id }, ["role", "entries"]);
        const orders = await strapi.services.orders.find({ user: id });
        const ordersReturn = await Promise.all(user.orders.map(async order => {
